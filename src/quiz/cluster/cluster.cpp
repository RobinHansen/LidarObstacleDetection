...
void proximity(KdTree *tree, const std::vector<std::vector<float>>& points, int i,
				std::vector<int> &cluster, std::vector<bool> &processed, float distanceTol) {
	processed[i] = true;
	cluster.push_back(i);
  	for (auto j : tree->search(points[i], distanceTol))
		if (!processed[j])
			proximity(tree, points, j, cluster, processed, distanceTol);
}
...
{	std::vector<bool> processed(points.size(), false);
	...
	for (int i = 0; i < points.size(); ++i) {
		if (!processed[i]) {
			...
			proximity(tree, points, i, cluster, processed, distanceTol);
...
int main ()
{
	...
	std::vector<std::vector<float>> points = { {-6.2,7}, {-6.3,8.4}, {-5.2,7.1}, {-5.7,6.3}, {7.2,6.1},
											   {8.0,5.3}, {7.2,7.1}, {0.2,-7.1}, {1.7,-6.9}, {-1.2,-7.2}, {2.2,-8.9} };
